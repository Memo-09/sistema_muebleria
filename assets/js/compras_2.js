...
function guardarCompra(event) {
    ...
    fetch(baseURL +"/funciones_base/insertar_compra.php", {
        method: "POST",
        body: datos
    })
...
function togglePopupEliminarCompra(claveCompra) {
    const popup = document.getElementById("popupEliminarCompra");
    const sidebar = document.getElementById("sidebar");
    const closePopup = document.getElementById("closePopupEliminarCompra");
    const productoIdLabel = document.getElementById("compraLabel");

    productoIdLabel.textContent = `${claveCompra}`;
    ...
    const btnEliminarProducto = document.getElementById("btnEliminarCompra");
    btnEliminarProducto.onclick = function () {
        eliminarCompra(claveCompra); // Llamar a la función de eliminación
    };
}


function eliminarCompra(claveCompra) {
    // Confirmar antes de eliminar
    if (!confirm(`¿Estás seguro de que deseas eliminar la Compra con ID ${claveCompra}?`)) {
        return;
    }
    ...
    xhr.open("POST", baseURL + "/funciones_base/eliminar_compra.php", true);
    ...
    xhr.send(`claveCompra=${claveCompra}`);
}


function generarListaCompras() {
    var url = "lista_compras.php"; // URL del script que genera el PDF
