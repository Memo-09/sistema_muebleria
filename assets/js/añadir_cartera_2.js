    ...
    function agregarFilasATabla(datos) {
        ...
        datos.forEach(function (fila) {
            ...
            row.querySelector(".btn-add-anticipo").addEventListener("click", function (event) {
                ...
                manejarYGuardarAnticipo(anticipo, restanteActual, row, idVenta);
            });
        });
    }

    // Función adicional para manejar el cálculo y actualización del restante
    function manejarYGuardarAnticipo(anticipo, restanteActual, row, idVenta) {
        ...
        // Llamada a la función para ejecutar el procedimiento almacenado
        var baseURL = window.location.pathname.split("/").slice(0, -1).join("/"); // Obtener la ruta base
    
        var xhr = new XMLHttpRequest();
        xhr.open("POST", baseURL + "/funciones_base/actualizar_restante_otro.php", true);
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                // Si la respuesta contiene un mensaje de error (anticipo ya registrado en esa fecha)
                if (response.indexOf("No se puede registrar el anticipo") !== -1) {
                    alert(response); // Mostrar el mensaje de error
                    row.querySelector("input[placeholder='Anticipo']").value = '';
                } else {
                    // Si la respuesta es exitosa, realizamos la resta
                    ...
                    alert("Anticipo guardado y restante actualizado correctamente.");
                }
            }
        };
    
        // Enviar los datos al servidor
        xhr.send("idVenta=" + idVenta + "&anticipo=" + anticipo + "&nuevoRestante=" + restanteActual);
    }
});
