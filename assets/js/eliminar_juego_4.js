function eliminarJuego(nombre, caracteristicas) {
    ...
    xhr.onload = function () {
        if (xhr.status === 200) {
            console.log(xhr.responseText);  // Mostrar lo que devuelve el servidor
            alert(xhr.responseText); // Mostrar la respuesta completa del servidor

            // Verificar si la respuesta indica que la eliminación fue exitosa
            if (xhr.responseText.includes('El juego y sus productos asociados fueron eliminados correctamente.')) {
