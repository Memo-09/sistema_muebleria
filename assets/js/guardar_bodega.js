...
function guardarBodega(event) {
    // Prevenir que el formulario se envíe de forma tradicional
    event.preventDefault();

    // Recoger el valor del campo de sucursal
    var bodega = document.querySelector('#ubicacionBodega').value.trim();

    if (!bodega) {
        alert("El campo Bodega no puede estar vacío.");
        return;
    }
    ...
    xhr.open("POST", baseURL + "/funciones_base/insertar_bodega.php", true); // Ajusta la ruta al archivo PHP correcto
    xhr.onload = function () {
        if (xhr.status === 200) {
            try {
            ...
            alert("Hubo un error al insertar la Bodega.");
        }
    };
