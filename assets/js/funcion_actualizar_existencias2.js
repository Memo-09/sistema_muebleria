...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const Bodegas = document.querySelector('#claveBodega').value;
    ...
    tabla2.addEventListener("click", function (event) {
        ...
        if (btn.classList.contains("actualizar-btn")) {
            ...
            actualizarExistencias(Bodegas, claveProducto, nuevasExistencias)
                .then((mensaje) => {
                    alert(mensaje);
                })
                .catch((error) => {
    ...
    async function actualizarExistencias(bodegas, clave, existencias) {
        const formData = new FormData();
        formData.append("idBodegas", bodegas);
        formData.append("claveProducto", clave);
        formData.append("nuevasExistencias", existencias);

        try {
            const response = await fetch(baseURL+"/funciones_base/actualizar_existencias2.php", {
                method: "POST",
                body: formData,
            });
