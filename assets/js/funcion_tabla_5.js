...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var bodega = document.querySelector('#claveBodega').value;
    ...
    function enviarProducto(clave, nuevaFila) {
        ...
        fetch(baseURL + "/funciones_base/archivo_php_agre_bod.php", {
            method: "POST",
            body: formData
        })
            ...
            .then(data => {
                ...
                if (data.trim() === "Producto ya registrado en esta bodega") {
    ...
    tabla2.addEventListener("click", function (event) {
        if (event.target.classList.contains("eliminar-btn")) {
            ...
            const bodega = document.querySelector('#claveBodega').value; // Obtener la sucursal

            // Llamar a la función para eliminar el producto desde la base de datos
            eliminarProducto(bodega, clave, fila);
        }
    });

    // Función para eliminar el producto desde la base de datos
    function eliminarProducto(bodega, clave, fila) {
        // Crear un objeto FormData para enviar los datos
        const formData = new FormData();
